...
class Header {
	...
	initSelectProject() {
		let selProject = this.el.find('#selProject');
		if (!selProject.length) return;
		//select event
		selProject.find('.dropdown-item').click((e) => {
			e.preventDefault();
			const _tempData = {
				id: SAILS_LOCALS.me.id,
				emailAddress: SAILS_LOCALS.me.emailAddress,
				projectActive: $(e.currentTarget).data('idProject'),
				pondActive: null
			};
			this.callApiEditUser(_tempData);
		});
	}
	callApiEditUser(data) {
		Cloud.editUser.with(data).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
			if (err) {
				console.log('==================================');
				console.log('error', );
				console.log('==================================');
